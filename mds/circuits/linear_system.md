...
$$\omega_0 = \frac{1}{RC}$$ の周波数で $$1/\sqrt{2}$$ (すなわち-3dB) の減衰、$$\omega \ll \omega_0$$ では周波数に比例する $$\omega / \omega_0$$ の透過率となります。
...
物理的に実現できる系では因果律により、出力は入力が加わる以前の時刻には現れません。
よって $$t<0$$ では $$g(t) = 0$$ です。  
...
$$t>0$$ かつ $$\sin \theta \geq 0 \ (\theta: 0 \rightarrow \pi)$$ より、(A.6)式の最右辺はゼロとなります。
よって
...
## 補遺C: CR回路がハイパスフィルターとなる直感的理解

簡単のため、CR回路に次のような直流電源をつなぎ、それをスイッチでON/OFFできるようにしましょう。

![](/assets/images/circuits/linear_system_05.png)  

このスイッチによりON/OFFを素早く切り替える (高周波) と、コンデンサーに電荷が貯まり切ることなく、抵抗器部分に電流が流れ続けます。
しかしON/OFFの切り替えをゆっくりと行う (低周波) と、コンデンサーに電荷が貯まり切ってしまい、それ以上は抵抗器部分に電流が流れなくなります。
同様に考えることで、次図のようなRC回路がローパスフィルターとなることも理解できます。

![](/assets/images/circuits/linear_system_06.png)  

ON/OFFの切り替えをゆっくり行うことで、コンデンサーに電荷が十分蓄電されます。
これにより、$$V_\mathrm{out}$$ は十分大きな値になることができます。
しかしON/OFFの切り替えを素早く行うと、コンデンサーに電荷が貯まり切る前に電流の向きが変わってしまい、十分な蓄電が行われません。
よって $$V_\mathrm{out}$$ は大きな値になることができず、高周波を通さないことがわかります。
