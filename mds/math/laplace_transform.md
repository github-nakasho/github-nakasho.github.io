...
$$\tau$$ を定義するために出てきた $$\zeta$$ は摩擦係数を表します。
これをラプラス変換を用いて解いてみましょう。
(9)式より
...
= \frac{v(0)}{s + \frac{1}{\tau}} + \frac{F(s)}{s + \frac{1}{\tau}} \tag{39}
$$

ここで $$v, f$$ のラプラス変換をそれぞれ $$V, F$$ としました。
ラプラス変換により、初期条件の寄与と外力に寄与を分離することができました。
(39)式をラプラス逆変換しましょう。
(17)式より
...
を得ます。
これは外力が存在しない場合、摩擦により初期の運動が時間とともに減衰する様子を表しています。
...
のようになります。
初期の速度 $$v(0)$$ が時間 $$t$$ の経過により緩和したもの $$v(0) e^{-t/\tau}$$ と、時刻 $$t'$$ の に加えられた外力 $$f(t')$$ が $$t-t'$$ の時間経過により緩和したものを $$0 \leq t' \leq t$$ で積分したものを足し合わせた形となっています。
(42)式の $$f(t')$$ に具体的な形を入れることで、その系における $$v(t)$$ を求めることができます。

{: .note}
外力 $$f(t)$$ が流体分子から受ける力の絶えず変動する部分 (揺動力) である場合、特に $$\overline{f(t)} = 0, \overline{f(t) f(t')} = 2\zeta k_B T \delta (t-t')$$ のような平均値と相関関数を持つ場合、(38)式の微粒子の運動方程式をランジュバン (Langevin) 方程式と呼びます。そしてこのような微粒子の熱運動をブラウン運動と呼びます。
