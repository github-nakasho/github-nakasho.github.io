...
= \sqrt{\frac{(2\pi)^N}{\rm{det}(A)}} \ (x \in R^N) \tag{1.11}
$$
...
エネルギーから、全てのスピンは揃います。しかしエントロピー$$S$$からは、温度を上げると状態の乱雑さが上昇し、向きは不揃いになるはずです。このことから温度を上昇させると、スピンの向きが揃った状態から不揃いな状態に遷移する特別な温度の存在が示唆されます。
...
これは熱力学でのヘルムホルツエネルギー$$F = U - TS$$を再現していることがわかります。途中、$$N = e^{\log N}$$は、$$N \rightarrow \infty$$においては$$N$$に比例する項に比べて無視できるとしました。
...
物理では$$N$$はアボガドロ数程度ですが、情報科学においては$$N \sim \mathcal{O}(100~1000)$$です。これでも十分に統計力学が使えることが実験や数値計算で示されています。  
