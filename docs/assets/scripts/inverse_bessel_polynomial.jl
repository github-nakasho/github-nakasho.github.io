...
for n in ns
    yn = zeros(len_x)
    for k in 0:n
        yn .+= factorial(n + k) ./ (factorial(n - k) * factorial(k)) .* xs .^ (n-k) ./ (2 ^ k)
    end
    y[n+1, :] = yn
end

p = plot(xs, y[1, :], linewidth=3, xlabel=L"x", ylabel=L"\theta_n(x)", title="Bessel polynomials", label=L"n=0", left_margin=5Plots.mm, ylim=[-5, 15], xlim=[x_min, x_max])
...
savefig(p, "inverse_bessel_polynomials.png")