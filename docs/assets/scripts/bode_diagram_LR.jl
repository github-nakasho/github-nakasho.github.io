...
p2 = plot(omegas, argG, linewidth=3, xlabel=L"\omega / \omega_0", ylabel=L"\mathrm{arg}(G) \ (\mathrm{degree})", legend=false, xscale=:log10, ylims=[-90.0, 0.0], left_margin=5Plots.mm)
plt = plot(p1, p2, layout=(2, 1), size=(500, 700))
savefig(plt, "bode_diagram_LR.png")